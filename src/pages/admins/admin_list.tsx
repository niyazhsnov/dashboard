...
import {
  ...
  const AddUserModal: FC = function () {
    ...
    const handleSubmit = async () => {
      ...
      try {
        await axios.post('https://telebe360.elxanhuseynli.com/api/admin', formData, {
  ...
  const AllUsersTable: FC = function () {
    ...
    useEffect(() => {
      axios.get<User[]>('https://telebe360.elxanhuseynli.com/api/admin', {
        headers: { Authorization: `Bearer ${Cookies.get('token')}` }
      })
        .then(response => {
  ...
  const EditUserModal: FC<{ user: User }> = function ({ user }) {
    ...
    const [password, setPassword] = useState('');  // setPasswordCurrent yerine setPassword olmalı.
    const [error, setError] = useState<string | null>(null);
    ...
    const handleSave = async () => {
      ...
      try {
        await axios.put(`https://telebe360.elxanhuseynli.com/api/admin/${user.id}`, {
          name,
          email,
          password
        }, {
  ...
  const DeleteUserModal: FC<DeleteUserModalProps> = function ({ userId, onDelete }) {
    ...
    const handleDelete = async () => {
      try {
        await axios.delete(`https://telebe360.elxanhuseynli.com/api/admin/${userId}`, {
          headers: { Authorization: `Bearer ${Cookies.get('token')}` }
        });
